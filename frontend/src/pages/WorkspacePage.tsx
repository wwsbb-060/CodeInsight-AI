...
import { Card, Input, Button, Table, Tag, Space, Popconfirm, App } from 'antd';
...
export default function WorkspacePage() {
  ...
  const handleImport = () => {
    ...
    createRepo.mutate({ url: url.trim() }, {
      onSuccess: (res) => {
        if (res.code === 200) {
    ...
    },
    {
      ...
      render: (_: unknown, record: RepositoryVO) => (
        <Space>
          ...
          <Popconfirm title="确定删除此仓库？" onConfirm={() => deleteRepo.mutate(record.id)}
    ...
    <>
      <Card style={{ marginBottom: 16 }}>
        <Space.Compact style={{ width: '100%' }}>
          <Input placeholder="输入 GitHub 仓库地址" value={url}
            onChange={(e) => setUrl(e.target.value)} onPressEnter={handleImport} size="large" />
          <Button type="primary" icon={<PlusOutlined />} onClick={handleImport}
            loading={createRepo.isPending} size="large">导入</Button>
        </Space.Compact>
      </Card>

      <Card title="我的仓库" extra={
        <Button icon={<ReloadOutlined />} onClick={() => refetch()} loading={isLoading}>
          刷新
        </Button>
      }>
        <Table columns={columns} dataSource={repos} rowKey="id" loading={isLoading}
          pagination={false}
